...
void rotation_matrix_T(void){
    Mat.DCM_T[0][0] = Mat.DCM[0][0]; //1.0f - 2.0f * q2*q2 - 2.0f * q3*q3;
    Mat.DCM_T[0][1] = Mat.DCM[1][0]; //2.0f * (q1*q2 +q0*q3);    
    Mat.DCM_T[0][2] = Mat.DCM[2][0]; //2.0f * (q1*q3 -q0*q2); 
    
    Mat.DCM_T[1][0] = Mat.DCM[0][1]; //2.0f * (q1*q2 -q0*q3);
    Mat.DCM_T[1][1] = Mat.DCM[1][1]; //1.0f - 2.0f * q1*q1 - 2.0f * q3*q3;  
    Mat.DCM_T[1][2] = Mat.DCM[2][1]; //2.0f * (q2*q3 + q0*q1);    
    
    Mat.DCM_T[2][0] = Mat.DCM[0][2]; //2.0f * (q1*q3 + q0*q2);
    Mat.DCM_T[2][1] = Mat.DCM[1][2]; //2.0f * (q2*q3 - q0*q1);
    Mat.DCM_T[2][2] = Mat.DCM[2][2]; //1.0f - 2.0f * q1*q1 - 2.0f * q2*q2;   
}
